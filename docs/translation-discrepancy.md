...
`--constructions` declares that the baseline must use all constructions currently enabled in NRDB. This retains already validated grammatical knowledge, so a known construction elsewhere in the sentence is not deliberately disabled while investigating a target morpheme. The setting is recorded in the discovery artifact.
...
The check reruns exactly the frozen baseline rows against the constructions enabled at check time. When the baseline was created with `--constructions`, this compares the earlier enabled knowledge against the expanded enabled table after target records have been validated and imported. It defaults to the baseline translation model and rejects a different `--translation-model`, since changing the generator would confound the construction comparison. The discrepancy model may be changed because it judges the baseline and current translations together in the same repair comparison.

Frozen-gold translation uses the same three-attempt malformed-JSON retry policy as ordinary direct translation.
